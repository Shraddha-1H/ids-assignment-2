...
st.write("Enter the phone specifications below to predict its price range (0 = Low, 1 = Medium, 2 = High, 3 = Very High).")
...
if submitted:
    ...
    row = {
        "battery_power": battery_power, "blue": yn[blue], "clock_speed": clock_speed,
        "dual_sim": yn[dual_sim], "fc": fc, "four_g": yn[four_g], "int_memory": int_memory,
        "m_dep": m_dep, "mobile_wt": wt_map[mobile_wt], "n_cores": n_cores, "pc": pc,
        "px_height": px_height, "px_width": px_width, "ram": ram, "sc_h": sc_h, "sc_w": sc_w,
        "talk_time": talk_time, "three_g": yn[three_g], "touch_screen": yn[touch_screen], "wifi": yn[wifi],
        "px_area": px_height * px_width, "screen_area": sc_h * sc_w,
    }
    ...
    label_map = {0: "Low (0)", 1: "Medium (1)", 2: "High (2)", 3: "Very High (3)"}
    st.success(f"Predicted Price Range: **{label_map[pred]}**")

    if hasattr(model, "predict_proba"):
        proba = model.predict_proba(X_new)[0]
        st.bar_chart(pd.Series(proba, index=[label_map[i] for i in model.classes_]))
